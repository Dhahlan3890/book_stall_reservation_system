...
def _stall_report_dict(stall_obj) -> Dict[str, Any]:
    """Internal: build a stall report dict with reservation info (no logic change)."""
    data = stall_obj.to_dict()
    active_reservation = Reservation.query.filter_by(
        stall_id=stall_obj.id,
        status='confirmed'
    ).first()

    if active_reservation:
        data['reserved'] = True
        data['reserved_by'] = active_reservation.user.business_name
        data['reserved_date'] = active_reservation.confirmed_at.isoformat()
        data['qr_code'] = active_reservation.qr_code
        data['qr_data'] = active_reservation.qr_data
    else:
        data['reserved'] = False
        data['reserved_by'] = None
        data['reserved_date'] = None
        data['qr_code'] = None
        data['qr_data'] = None
...
@jwt_required()
def get_stalls_report() -> tuple:
    """Get detailed stall information"""
    stalls = Stall.query.all()

    stalls_data: List[Dict[str, Any]] = []
    for stall in stalls:
        stalls_data.append(_stall_report_dict(stall))
...
@jwt_required()
def get_occupancy_analytics() -> tuple:
...
@jwt_required()
def get_revenue_analytics() -> tuple:
    """Get revenue analytics"""
    total_revenue = (
        db.session.query(db.func.sum(Stall.price))
        .join(Reservation)
        .filter(Reservation.status == 'confirmed')
        .scalar() or 0
    )

    revenue_by_size: Dict[str, Any] = {}
    for size in ['small', 'medium', 'large']:
        revenue = (
            db.session.query(db.func.sum(Stall.price))
            .filter(Stall.size == size)
            .join(Reservation)
            .filter(Reservation.status == 'confirmed')
            .scalar() or 0
        )
...
@jwt_required()
def cancel_reservation_admin(reservation_id: int) -> tuple:
...
@jwt_required()
def approve_reservation(reservation_id: int) -> tuple:
...
@jwt_required()
def reject_reservation(reservation_id: int) -> tuple:
    ...
    try:
        ...
        reason = json_data.get('reason', 'No reason provided') if json_data else 'No reason provided'
